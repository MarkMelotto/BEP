...
for test in tests:
    ...
    data.append({"resistor":resistor, "distance_lamp_to_box":distance_lamp_to_box,
                 "distance_box_to_detector":distance_box_to_detector, "humidity":humidity, "dark_voltage":dark_voltage,
...
for i in range(len(data)):
    ...
    axs[i].set_title(f"Voltage measure with the following parameters:\n"
        f"Resistance = {data[i]['resistor']/1000} kOhm, distance lamp-box-sensor: {data[i]['distance_lamp_to_box']}-{data[i]['distance_box_to_detector']} cm")
    axs[i].grid()
